 ...
 *   Author: Federico Negri Luca Rosafalco <federico.negri@epfl.ch>
 */

#include "LinearElasticMaterialSpatial.h"
...
void LinearElasticMaterialSpatial_forces(mxArray* plhs[], const mxArray* prhs[])
{
    ...
    int NumNodes          = (int)(mxGetM(prhs[2]) / dim);

	/*FILE * fp;
	FILE * fp2;*/

    double* U_h   = mxGetPr(prhs[2]);
	...
	double* mu     = mxGetPr(prhs[10]);
	double* lambda = mxGetPr(prhs[11]);


	/*fp = fopen("C:/Users/Rosafalco/Documents/Polimi/3_Dottorato/Codici/Matlab/Quarteroni_Manzoni_Negri/pippo2.out","w");
	fp2 = fopen("C:/Users/Rosafalco/Documents/Polimi/3_Dottorato/Codici/Matlab/Quarteroni_Manzoni_Negri/comparisonLEMS.out", "w");
	fprintf(fp,"noe = %u NumQuadPoints=%u  i2=%u \n", noe, NumQuadPoints, i2);*/


    /* Assembly: loop over the elements */
    int ie;

#pragma omp parallel for shared(invjac,detjac,elements,myRrows,myRcoef,U_h,mu,lambda) private(gradphi,F,EPS,dP,P_Uh,GradV,GradU,GradUh,ie,k,l,q,d1,d2) firstprivate(phi,gradrefphi,w,numRowsElements,nln2,nln,NumNodes,Id)
            ...
            for (i_c = 0; i_c < dim; i_c = i_c + 1 )
            {
                ...
                for (q = 0; q < NumQuadPoints; q = q + 1 )
                {
                    ...
                    for (d1 = 0; d1 < dim; d1 = d1 + 1 )
                    {
                        for (d2 = 0; d2 < dim; d2 = d2 + 1 )
                        {
                            P_Uh[d1][d2] = 2 * mu[ie + q*noe] * EPS[d1][d2] + lambda[ie+q*noe] * trace * Id[d1][d2];
							/*fprintf(fp, "indici ie=%u q=%u d1=%u d2=%u  \n", ie, q, d1, d2);
							fprintf(fp, "lambdaSpatial[ie+q*noe] = %f \n", lambdaSpatial[ie + q*noe]);
...
void LinearElasticMaterialSpatial_jacobian(mxArray* plhs[], const mxArray* prhs[])
{
	...
	/*fp3 = fopen("C:/Users/Rosafalco/Documents/Polimi/3_Dottorato/Codici/Matlab/Quarteroni_Manzoni_Negri/pippo_gradLEMS.out", "w");
	...
	fprintf(fp3, "noe = %u NumQuadPoints=%u  i2=%u \n", noe, NumQuadPoints, i2);*/

#pragma omp parallel for shared(invjac,detjac,elements,myAcols,myArows,myAcoef,U_h,mu,lambda) private(gradphi,F,EPS,dP,P_Uh,GradV,GradU,GradUh,ie,k,l,q,d1,d2) firstprivate(phi,gradrefphi,w,numRowsElements,nln2,nln,NumNodes,Id)
                    ...
                    for (j_c = 0; j_c < dim; j_c = j_c + 1 )
                    {
                        ...
                        for (q = 0; q < NumQuadPoints; q = q + 1 )
                        {
                            ...
                            for (d1 = 0; d1 < dim; d1 = d1 + 1 )
                            {
                                for (d2 = 0; d2 < dim; d2 = d2 + 1 )
                                {
                                    dP[d1][d2] = 2 * mu[ie + q*noe] * EPS[d1][d2] + lambda[ie + q*noe] * trace * Id[d1][d2];
									/*fprintf(fp3, "indici ie=%u q=%u d1=%u d2=%u  \n", ie, q, d1, d2);
									fprintf(fp3, "lambdaSpatial[ie+q*noe] = %f \n", lambdaSpatial[ie + q*noe]);
...
#pragma omp parallel for shared(invjac,detjac,elements,myAcols,myArows,myAcoef,U_h,mu,lambda) private(ie,k,l,q,d1,d2) firstprivate(phi,gradrefphi,w,numRowsElements,nln2,nln,NumNodes)
        ...
        for (q = 0; q < NumQuadPoints; q = q + 1 )
        {
            ...
            for (a = 0; a < nln; a = a + 1 )
            {
                ...
                for (b = 0; b < nln; b = b + 1 )
                {
                    ...
                    aloc[a][0][b][0] += ( gradphi[q][0][a]*(lambda[ie + q*noe]*gradphi[q][0][b] + 2.0*mu[ie + q*noe]*gradphi[q][0][b]) + mu[ie + q*noe]*gradphi[q][1][b]*gradphi[q][1][a] + mu[ie + q*noe]*gradphi[q][2][b]*gradphi[q][2][a] ) * w[q];

                    aloc[a][0][b][1] += ( lambda[ie + q*noe]*gradphi[q][1][b]*gradphi[q][0][a] + mu[ie + q*noe]*gradphi[q][0][b]*gradphi[q][1][a] ) * w[q];

                    aloc[a][0][b][2] += ( lambda[ie + q*noe]*gradphi[q][2][b]*gradphi[q][0][a] + mu[ie + q*noe]*gradphi[q][0][b]*gradphi[q][2][a] ) * w[q];

                    aloc[a][1][b][0] += ( lambda[ie + q*noe]*gradphi[q][0][b]*gradphi[q][1][a] + mu[ie + q*noe]*gradphi[q][1][b]*gradphi[q][0][a] ) * w[q];

                    aloc[a][1][b][1] += ( gradphi[q][1][a]*(lambda[ie + q*noe]*gradphi[q][1][b] + 2.0*mu[ie + q*noe]*gradphi[q][1][b]) + mu[ie + q*noe]*gradphi[q][0][b]*gradphi[q][0][a] + mu[ie + q*noe]*gradphi[q][2][b]*gradphi[q][2][a] ) * w[q];

                    aloc[a][1][b][2] += ( lambda[ie + q*noe]*gradphi[q][2][b]*gradphi[q][1][a] + mu[ie + q*noe]*gradphi[q][1][b]*gradphi[q][2][a] ) * w[q];

                    aloc[a][2][b][0] += ( lambda[ie + q*noe]*gradphi[q][0][b]*gradphi[q][2][a] + mu[ie + q*noe]*gradphi[q][2][b]*gradphi[q][0][a] ) * w[q];

                    aloc[a][2][b][1] += ( lambda[ie + q*noe]*gradphi[q][1][b]*gradphi[q][2][a] + mu[ie + q*noe]*gradphi[q][2][b]*gradphi[q][1][a] ) * w[q];

                    aloc[a][2][b][2] += ( gradphi[q][2][a]*(lambda[ie + q*noe]*gradphi[q][2][b] + 2.0*mu[ie + q*noe]*gradphi[q][2][b]) + mu[ie + q*noe]*gradphi[q][0][b]*gradphi[q][0][a] + mu[ie + q*noe]*gradphi[q][1][b]*gradphi[q][1][a] ) * w[q];
        ...
        for (q = 0; q < NumQuadPoints; q = q + 1 )
        {
            ...
            for (a = 0; a < nln; a = a + 1 )
            {
                ...
                for (b = 0; b < nln; b = b + 1 )
                {
                    ...
                    aloc[a][0][b][0] += ( gradphi[q][0][a]*(lambda[ie + q*noe]*gradphi[q][0][b] + 2.0*mu[ie + q*noe]*gradphi[q][0][b]) + mu[ie + q*noe]*gradphi[q][1][b]*gradphi[q][1][a] ) * w[q];

                    aloc[a][0][b][1] += ( lambda[ie + q*noe]*gradphi[q][1][b]*gradphi[q][0][a] + mu[ie + q*noe]*gradphi[q][0][b]*gradphi[q][1][a] ) * w[q];

                    aloc[a][1][b][0] += ( lambda[ie + q*noe]*gradphi[q][0][b]*gradphi[q][1][a] + mu[ie + q*noe]*gradphi[q][1][b]*gradphi[q][0][a] ) * w[q];

                    aloc[a][1][b][1] += ( gradphi[q][1][a]*(lambda[ie + q*noe]*gradphi[q][1][b] + 2.0*mu[ie + q*noe]*gradphi[q][1][b]) + mu[ie + q*noe]*gradphi[q][0][b]*gradphi[q][0][a] ) * w[q];
                }
            }
        }
...
#pragma omp parallel for shared(invjac,detjac,elements,U_h,mu,lambda) private(gradphi,F,EPS,GradUh,ie,k,l,q,d1,d2) firstprivate(phi,gradrefphi,w,numRowsElements,nln2,nln,NumNodes,Id)
    ...
    for (ie = 0; ie < noe; ie = ie + 1 )
    {
        ...
        for (d1 = 0; d1 < dim; d1 = d1 + 1 )
        {
            for (d2 = 0; d2 < dim; d2 = d2 + 1 )
            {
                Sigma[ie+(d1+d2*dim)*noe] = 2 * mu[ie + q*noe] * EPS[d1][d2] + lambda[ie + q*noe] * trace * Id[d1][d2];  /*attenzione - da verificare*/
...
void LinearElasticMaterialSpatial_stiffness(mxArray* plhs[], const mxArray* prhs[])
{
    ...
    for (d1 = dim; d1 < i1; d1 = d1+1)
    {
        dim_circle[d1] = (d1-dim);
    }


#pragma omp parallel for shared(invjac,detjac,elements,myKrows,myKcols,myKcoef_mu,myKcoef_lambda,mu,lambda,dim_circle) private(gradphi,ie,k,q,d1,d2,d3,tmp_mu,tmp_lambda) firstprivate(gradrefphi,w,numRowsElements,NumQuadTot,nln2,nln2_dim,nln,NumNodes,LocalStiff_mu,LocalStiff_lambda)
			...
			for (b = 0; b < nln; b = b + 1 )
			{
				...
				for (d1 = 0; d1 < dim; d1 = d1 + 1 )
				{
			    	for (d2 = 0; d2 < dim; d2 = d2 + 1 )
					{
						...
						for (q = 0; q < NumQuadPoints; q = q + 1 )
	                	{
							tmp_mu     = tmp_mu + mu[ie + q*noe] * gradphi[d1][b][q] * gradphi[d2][a][q]* w[q];
							tmp_lambda = tmp_lambda + lambda[ie + q*noe] * gradphi[d1][a][q] * gradphi[d2][b][q]* w[q];
						}
				...
				for (d1 = 0; d1 < dim; d1 = d1 + 1 )
				{
					for (d2 = 1; d2 < dim; d2 = d2 +1 )
					{
						...
						for (q = 0; q < NumQuadPoints; q = q + 1 )
	                	{
							tmp_mu = tmp_mu + mu[ie + q*noe] *gradphi[d1][a][q] * gradphi[d1][b][q]* w[q];
						}
