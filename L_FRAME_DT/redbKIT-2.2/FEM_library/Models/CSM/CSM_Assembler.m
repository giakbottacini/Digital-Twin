...
%    compute_thermal_term_elastic - assemble thermal term matrix
...
classdef CSM_Assembler < handle
    ...
    methods
        ...
        %========================================================================== 
        %% Compute two components thermal term matrix for linear elastic behaviour
        function G = compute_thermal_term_elastic( obj, t, param )
        ...
        if strcmp(obj.M_MaterialModel, 'LinearSpatial')
            ...
            % 
                ...
                % C_OMP assembly, returns matrices in sparse vector format        	
                [rowG, colG, coefG_mu, coefG_lambda] = ...
                            CSM_assembler_C_omp(obj.M_MESH.dim, [obj.M_MaterialModel,'_thermal'], ...
                            obj.M_MESH.elements, obj.M_FE_SPACE.numElemDof, obj.M_FE_SPACE.quad_weights, ...
                            obj.M_MESH.invjac, obj.M_MESH.jac, obj.M_FE_SPACE.phi, obj.M_FE_SPACE.dphi_ref, ...
                            obj.M_MESH.numNodes, mu, lambda);
        ...
        % Build sparse matrix
        G_mu     = GlobalAssemble(rowG, colG, coefG_mu, (obj.M_MESH.numNodes*obj.M_FE_SPACE.numComponents), (obj.M_MESH.numNodes));
        G_lambda = GlobalAssemble(rowG, colG, coefG_lambda, (obj.M_MESH.numNodes*obj.M_FE_SPACE.numComponents), (obj.M_MESH.numNodes));

        G = 2 .* param(1) .* G_mu + 2 .* param(2) .* G_lambda;
	
        if obj.M_MESH.dim == 2	                        
            G     = G .* obj.M_DATA.Thickness .* param(9);            
        end
