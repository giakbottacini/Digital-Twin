    ...
    case 'CSM'
        ...
        for d = 1 : MESH.dim
            ...
            if ~isempty(type_Dirichlet)
                ...
                Dirichlet_side          = [];
                for kk = 1 : nDir
                    %L: ritrova tutti i nodi associati alla condizione al
                    %contorno considerata (e cioè al lato valutato e alla
                    %direzione presa in esame). Questi sono infatti
                    %segnalati nella quinta riga(=bc_flag_row) di
                    %'MESH.boundaries'
                    this_Dirichlet_side     = find(MESH.boundaries(bc_flag_row,:) == type_Dirichlet(kk));
                    this_Dirichlet_dof      = MESH.boundaries(1:MESH.numBoundaryDof, unique( this_Dirichlet_side ) );  %unique() permette di valutare un'unica volta quanto scritto nel vettore tra parentesi
                    MESH.DiriDof_CompFlag{d,kk}  = unique(this_Dirichlet_dof(:));
                    Dirichlet_side          = [Dirichlet_side, this_Dirichlet_side];
                end
                %si potrebbe pensare che Dirichlet_side costituisca una
                %sorta di numerazione locale dei gradi di libertà sul
                %contorno, mentre Dirichlet_dof costituisca la numerazione
                %globale ad essa associata (entrambe queste informazioni
                %sono contenute in 'MESH.boundaries'.
                Dirichlet_side             = unique(Dirichlet_side);
                Dirichlet_dof              = MESH.boundaries(1:MESH.numBoundaryDof,Dirichlet_side);
