...
void mexFunction(int nlhs,mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    double* density = mxGetPr(prhs[6]);
    
    double LocalMass[nln][nln];
    int k, l, ie, q, iii, a, b;

    /* Assembly: loop over the elements */
            
    #pragma omp parallel for shared(detjac,elements,myMcols, myMrows, myMcoef, density) private(ie) firstprivate(phi, numRowsElements, nln2, LocalMass)
    for (ie = 0; ie < noe; ie = ie + 1 )
    {
        for (k = 0; k < nln; k = k + 1 )
        {
            for (l = 0; l < nln; l = l + 1 )
            {
                ...
                {
                    tmp = tmp + phi[k+q*nln] * phi[l+q*nln] * w[q] * density[ie+q*ie];
                }
                LocalMass[k][l] = tmp;
            }
        }
