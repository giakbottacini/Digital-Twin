    ...
    case 2
        ...
        switch fem
            ...
            case 'P1'
                ...
                %L: funzioni di forma per l'elemento utilizzato
                phi(1,:) = 1-x-y;
                phi(2,:) = x;
                phi(3,:) = y;
                %L: la matrice dphix contiene la derivata delle funzioni di
                %forma calcolare per ciascun punto d'integrazione
                dphix(1,:) = -1+0.*x;
                dphix(2,:) =  1+0.*x;
                dphix(3,:) =  0+0.*x;
                dphiy(1,:) = -1+0.*x;
        ...
        %dphi raccoglie in un unico operatore matematico (di tre
        %dimensioni) le derivate delle funzioni di forma valutate per i
        %punti di quadratura definiti precedentemente.
        dphi(:,:,1) = dphix;
        dphi(:,:,2) = dphiy;
