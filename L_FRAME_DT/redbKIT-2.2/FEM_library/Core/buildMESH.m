...
%L: permette di aumentare l'ordine di interpolazione proprio della mesh
%considerata (secondo quanto è contenuto in 'fem').
%Qualora si mantenesse inalterato ('P1' ordine di interpolazione linerare)
%nella struttura MESH ai nodi si faranno coinciedere i 'vertices' prima
%determinati (tutte le altre informazioni sono già copiate correttamente).
if ~strcmp(fem,'P1')
...
%L: si dovrebbe trattare della procedura con cui si stabilisce la mappatura
%tra gli elementi creati dalla mesh e il parent element (per la scrittura
%delle funzioni di forma). In particolare
%   -   MESH.jac contiene (per ogni triangolo della mesh) la sua matrice
%       jacobiana
%   -   MESH.invjac contiene l'inverso della sua matrice jacobiana
%   -   MESH.h contiene la sua dimensione maggiore
...
%L: restituisce i punti di quadratura per il parent element della tipologia
%di elementi considerati (in base alla 'dim' e alla 'quad_order'
%specificati). Calcola i pesi, ma non li passa.
